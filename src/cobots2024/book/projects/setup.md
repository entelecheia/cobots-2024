...
먼저, [cobots-2024](https://github.com/chu-aie/cobots-2024.git) 저장소를 자신의 GitHub 계정으로 복제(Fork)합니다. 복제한 저장소의 URL은 `https://github.com/your_github_id/cobots-2024.git`와 같은 형식이 될 것입니다.
...
다음으로, 본인이 복제한 리포를 원본 리포([cobots-2024](https://github.com/chu-aie/cobots-2024.git))와 동기화합니다. 이를 위해 원본 리포를 `upstream`으로 추가하고, 최신 변경사항을 가져옵니다:

```bash
git remote add upstream https://github.com/chu-aie/cobots-2024.git \
    && git fetch upstream
```

- `git remote add upstream https://github.com/chu-aie/cobots-2024.git`: 원본 리포를 `upstream`이라는 이름으로 추가합니다.
