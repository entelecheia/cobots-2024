...
이 가이드는 협동로봇활용 수업을 위한 프로젝트 환경을 설정하는 방법을 자세히 설명합니다. 프로젝트를 시작하기 전에 아래 단계를 따라 환경을 설정해주세요.

## 리포 복제

먼저, [cobots-2024](https://github.com/entelecheia/cobots-2024.git) 저장소를 자신의 GitHub 계정으로 복제(Fork)합니다. 복제한 저장소의 URL은 `https://github.com/your_github_id/cobots-2024.git`와 같은 형식이 될 것입니다.

그 다음, 복제한 저장소를 로컬 컴퓨터로 클론합니다. 아래 명령을 터미널에서 실행하세요:

```bash
mkdir -p ~/workspace/projects \
    && git clone https://github.com/your_github_id/cobots-2024.git ~/workspace/projects/cobots-2024 \
    && cd ~/workspace/projects/cobots-2024
```

- `mkdir -p ~/workspace/projects`: `~/workspace/projects` 디렉토리를 생성합니다. 이 디렉토리는 프로젝트를 저장할 공간입니다.
- `git clone https://github.com/your_github_id/cobots-2024.git`: 복제한 저장소를 로컬로 클론합니다. `your_github_id` 부분은 본인의 GitHub 아이디로 대체해주세요.
- `cd ~/workspace/projects/cobots-2024`: 클론한 저장소의 디렉토리로 이동합니다.
...
다음으로, 본인이 복제한 리포를 원본 리포([cobots-2024](https://github.com/entelecheia/cobots-2024.git))와 동기화합니다. 이를 위해 원본 리포를 `upstream`으로 추가하고, 최신 변경사항을 가져옵니다:

```bash
git remote add upstream https://github.com/entelecheia/cobots-2024.git \
    && git fetch upstream
```

- `git remote add upstream https://github.com/entelecheia/cobots-2024.git`: 원본 리포를 `upstream`이라는 이름으로 추가합니다.
